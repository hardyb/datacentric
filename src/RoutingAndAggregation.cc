...
void handle_pubbreakage(control_data cd)
{

    // if we are THE source for this full data/context name
    // then put the packet in the queue and reinitiate a new advert
    // make sure you use a new seqno for advert & resending data
    trie* t = trie_lookup_longest_prefix_extra2(rd->top_state, incoming_packet.data);
    ...
    // if we are NOT THE source for this full data/context name
    // forward the packet on the obtain interface
    if( t && t->s->obtainInterfaces )
    {
        outgoing_packet.message_type = PUBBREAKAGE;
        outgoing_packet.length = incoming_packet.length;
        outgoing_packet.data = incoming_packet.data;
        outgoing_packet.path_value = 0;
        outgoing_packet.excepted_interface = UNKNOWN_INTERFACE;
...
{
   if ( !strcmp((const char*)str, "\x83\xFF\x6") )
   {
